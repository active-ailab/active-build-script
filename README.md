...
| bstyle 编译 | `active-bstyle` 独立调用工作区内 `build/cmd/linux32|linux64/bstylenc`，不会串入固件编译流程。 |
...
| Sensorhub 产物处理 | sensorhub 产物生成在 `build/out_hub/`；family/project 校验不依赖产品目录。 |
...
- 无参数运行时进入交互模式：选择 family、project、style 输入文件，默认输出为同目录同名 `.bstyle`，并可选择是否修改输出路径。
- `-i` 传给底层 `bstylenc -i`；未配置时，只会在当前目录存在唯一 `.style` 文件时自动推导，否则报错。
- `-o` 传给底层 `bstylenc -o`；未配置时由 `input.style` 生成同目录同名 `.bstyle`。
