...
- `version_explicit` 表示 BuildPlan 是否显式要求版本。BuildPlan JSON 中包含 `version` 但未写 `version_explicit` 时，CLI 会按 `true` 处理。新工程未显式传入版本时默认使用 `99.9`。
- 新工程版本格式只支持两段 `c.d` 或四段 `a.b.c.d` 数字版本。默认 `99.9` 或显式两段版本会在 `make ota` 上追加 `FW_VER_STRATEGY=os_global BUILD_FW_VER=<version>`；四段版本只追加 `BUILD_FW_VER=<version>`。
...
- 新工程只在 `make ota` 阶段追加版本变量；未显式传入版本时默认 `version=99.9`。
- 新工程默认值 `99.9` 或显式两段版本 `c.d` 会生成 `FW_VER_STRATEGY=os_global BUILD_FW_VER=<version>`，由项目拼接 OS 前两位。
